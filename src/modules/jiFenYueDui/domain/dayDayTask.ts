...
import { DayDayTaskCreated } from './events/dayDayTaskCreated'
...
export class DayDayTask extends AggregateRoot<IDayDayTaskProps> {
  ...
  public static create(props: IDayDayTaskProps, id?: UniqueEntityID): Result<DayDayTask> {
    ...
    const dayDayTask = new DayDayTask(defaultValues, id)

    const isNew = !!id === false
    if (isNew) {
      dayDayTask.addDomainEvent(new DayDayTaskCreated(dayDayTask))
    }
    return Result.ok<DayDayTask>(dayDayTask)
  }
}
